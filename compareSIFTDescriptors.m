function [indexPoints] =compareSIFTDescriptors(descriptors1, descriptors2)
[features1size]=size(descriptors1);
[features2size]=size(descriptors2);
indexPoints=[];
count=0;
for i = 1:features1size
    ...
    for j = 1:features2size
        ...
        distance = 0;
        for k = 1:128
            diff = descriptors1(i,k) - descriptors2(j,k);
...
%     disp(ratio);
    if ratio <= 0.6
        indexPoints = [indexPoints ; [i minindex1]];
%         [i minindex1]
        count = count +1;
    end
end
